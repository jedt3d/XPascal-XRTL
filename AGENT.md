...
- Current next slices after #58: issue `#41` researches FCL-Web vs `fphttpserver` and fork/credit strategy; issue `#42` designs provider-neutral `XRTL.Web` middleware inspired by Rack and Horse; issue `#47` continues the local-only Database feature stack with ORM next.
...
- After issue `#58`, SELECT query builder primitives are available. The next Database slice should define the smallest ORM layer on top of provider, migration, and query builder contracts.
