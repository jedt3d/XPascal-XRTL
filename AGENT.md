...
- Current next slices after #64: issue `#41` researches FCL-Web vs `fphttpserver` and fork/credit strategy; issue `#42` designs provider-neutral `XRTL.Web` middleware inspired by Rack and Horse. Further ORM work needs new focused follow-up issues for query/list support, validation hooks, typed adapters, migration helpers, or XRTL.Data bridge planning.
- Latest completed implementation slice: issue `#40` adds `XRTL.Database` v0 local SQLite over SQLDB and tests from accepted ADR `0008`; issue `#45` completes the macOS Apple Silicon SQLite validation rerun on merged `main`; issue `#48` adds public transactions and parameter binding from ADR `0009`; issue `#50` adds detached raw row/field/value mapping from ADR `0010`; issue `#52` adds app dataset iteration from ADR `0011`; issue `#54` adds the SQLite-only provider abstraction facade from ADR `0012`; issue `#56` adds apply-only migration primitives from ADR `0013`; issue `#58` adds SELECT query builder primitives from ADR `0014`; issue `#60` adds ORM mapping primitives from ADR `0015` and completes #47; issue `#62` adds the ORM V1 spec from ADR `0016`; issue `#64` and PR `#65` add the SQLite-only ORM V1 contract from ADR `0017`.
...
- After issue `#60`, the local-only `XRTL.Database` feature stack is complete. Issue `#62` completed the ORM V1 planning gate before write-side ORM behavior proceeds.
- For issue `#64` and ORM V1, keep execution SQLite-only; implement CRUD mapper, repositories, explicit Session/Unit of Work, schema/generated-SQL/session identity-map caching, and attachment metadata separated from storage/upload.
- Do not put file upload transport, raw file bytes, object-storage SDKs, or file-byte caching inside ORM. ORM may map attachment metadata and relationships only.
