...
- After issue `#60`, the local-only `XRTL.Database` feature stack is complete. Issue `#62` is the follow-up planning gate for ORM V1 before write-side ORM behavior proceeds.
- For ORM V1, keep execution SQLite-only; start with CRUD mapper, repositories, explicit Session/Unit of Work, schema/generated-SQL/session identity-map caching, and attachment metadata separated from storage/upload.
