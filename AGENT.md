...
- Current next slices after #60: issue `#41` researches FCL-Web vs `fphttpserver` and fork/credit strategy; issue `#42` designs provider-neutral `XRTL.Web` middleware inspired by Rack and Horse. The local-only Database feature stack in issue `#47` is complete after #60.
...
- After issue `#60`, the local-only `XRTL.Database` feature stack is complete. New Database work should open a specific follow-up issue for write-side ORM behavior, richer query builder features, or future network provider validation.
